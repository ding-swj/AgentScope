...
Usage:
  ...
  npm run agentscope -- summarize --input <trace-file> [--dry-run] [--compact] [--marker <marker>]
...
function renderVerification(actions, opts = {}) {
  const verificationActions = actions.filter((action) =>
    action.type === 'run_command' || action.type === 'test_failed' || action.type === 'test_passed',
  )
  ...
  for (const action of verificationActions) {
    ...
    const outputText = action.output?.trim() ?? ''
    const totalLines = outputText ? outputText.split(/\r?\n/).length : 0

    if (opts.compact) {
      lines.push(`- **${action.type}:** \`${String(label).replace(/`/g, '\\`')}\` (${totalLines} lines)`)
      continue
    }
...
function renderRunSummary(run, headingLevel = 2, opts = {}) {
  ...
  if (run.actions.length > 0) {
    ...
    if (omitted > 0) {
      const omittedActions = run.actions.slice(maxSummaryActions)
      const typeCounts = {}
      for (const action of omittedActions) {
      ...
      lines.push('', `...and ${omitted} more actions omitted (${parts.join(', ')})`, '')
    } else {
      lines.push('')
    }
  }

  lines.push(...renderVerification(run.actions, opts))
  lines.push('---')
...
function generateTraceSummary(trace, opts = {}) {
  ...
  lines.push('')
  for (const run of trace.runs) {
    lines.push('---', '', renderRunSummary(run, 3, opts), '')
  }
...
function parseSummarizeOptions(args) {
  const options = { input: '', dryRun: false, compact: false, marker: defaultSummaryMarker }
...
async function summarize(args) {
  ...
  if (!options.input) {
    console.error('Missing input file. Use: npm run agentscope -- summarize --input <trace-file> [--dry-run] [--compact]')
    exit(1)
  }
  ...
  const markdown = generateTraceSummary(raw, { compact: options.compact })
  if (options.dryRun) {
